...
app.get(
  ...
  catchAsync(async (req, res) => {
    ...
    if (user.role === "Admin") {
      userDetails = await Admin.findOne({ userId: user._id }).populate(
        "userId"
      );
    } else if (user.role === "Author") {
      userDetails = await Author.findOne({ userId: user._id }).populate(
        "userId"
      );
    } else if (user.role === "Reader") {
      userDetails = await Reader.findOne({ userId: user._id }).populate(
        "userId"
      );
    }
